...
double DantzigPrice::optimize(const std::vector<double>& duals, PricingBlockVector<GapPricing>& pricing, std::vector<double>& reduced_costs) {
    ...
    highs::parallel::for_each(0, _instance->machines, [&](HighsInt start, HighsInt end) {
        for (int m = start; m < end; ++m) {
            ...
            if (reduced_costs[m] > 1e-6) {
                ...
                while (true) {
                    for (int j = 0; j < _instance->jobs; ++j) {
                        tmpProfit[j] = _template[m][j] + hi_mu * obj[j];
                    }

                    auto opt = pricing[m].optimize(tmpProfit, hi_mu * offset);
                    ub = std::min(ub, std::floor(opt));

					double rc = sum(pricing[m].solution, offset, obj);

                    if (rc > 1e-6) {
                        best_rc = rc;
                        best = sum(pricing[m].solution, 0.0, _template[m]);
                        break;
                    }
				...
				best_solution.swap(pricing[m].solution);

                while (hi_mu - lo_mu > 1e-6 && best < ub) {
                    alpha[m] = 0.5 * (hi_mu + lo_mu);
                    for (int i = 0; i < _instance->jobs; ++i) {
                        tmpProfit[i] = _template[m][i] + alpha[m] * obj[i];
                    }

                    auto opt = pricing[m].optimize(tmpProfit, alpha[m] * offset);
                    ub = std::min(ub, std::floor(opt));
                    ...
                    if (rc <= 1e-6) {
                        ...
                        hi_mu = alpha[m];
                        double tmpl = sum(pricing[m].solution, 0.0, _template[m]);
                ...
                reduced_costs[m] = best_rc;
            }

            pricing[m].solution.swap(best_solution);
            pricing[m].solution.push_back(_instance->jobs + m);
        }
    });
