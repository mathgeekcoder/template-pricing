...
namespace dlib
{
	...
	template <typename T>
	inline void swap(memory_manager_stateless_kernel_1<T>& a, memory_manager_stateless_kernel_1<T>& b) {
		a.swap(b);
	}


	template <typename T, typename mem_manager = memory_manager_stateless_kernel_1<char>>
	...
	private:
		...
		// data members
		unsigned long sequence_size = 0;
		mutable node* current_node = nullptr;
		mutable unsigned long current_pos = 0;
		mutable bool at_start_ = true;
		mutable node* current_enumeration_node = nullptr;
		mutable unsigned long current_enumeration_pos = 0;
